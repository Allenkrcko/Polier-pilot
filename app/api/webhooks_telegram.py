...
async def telegram_inbound(
    ...
    if msg_type == MessageType.voice and file_id:
        try:
            ...
            from app.workers.tasks import process_telegram_voice_message

            default_queue().enqueue(process_telegram_voice_message, str(persisted.id))
            logger.info("Enqueued process_telegram_voice_message for id=%s", persisted.id)
        except Exception:
            logger.exception("Failed to enqueue Telegram voice for id=%s", persisted.id)
